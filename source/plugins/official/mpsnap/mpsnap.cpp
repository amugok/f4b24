...
static SimpleVector<LINE> TopEdge;
static SimpleVector<LINE> BottomEdge;
static SimpleVector<LINE> LeftEdge;
static SimpleVector<LINE> RightEdge;
...
BOOL CALLBACK EnumWindowsProc(HWND hWnd, LPARAM lParam)
{
	...
	if( (IsWindowVisible(hWnd))					&&	//  可視状態か
		...
		((SimpleVector<HWND> *)lParam)->push_back(hWnd);
	}
...
BOOL GetAllWindowEdge(HWND hWnd)
{
	...
	SimpleVector<HWND>::iterator ritr;
	
	for(ritr = wnd.end()-1; ritr != wnd.begin()-1; --ritr)
	{
...
{
	SimpleVector<LINE>::iterator itr;
	...
	switch(msg){
		...
		case WM_COMMAND:
			if(LOWORD(wp)==IDCANCEL){
				// サブクラス化の解除
				SetWindowLongPtr(hWnd, GWLP_WNDPROC, (LONG_PTR)hOldMiniPanelProc);
			}
			break;
	}
			...
			if(LOWORD(wp)==IDM_MINIPANEL){
				...
				while(hFound){
					if(fpi.hParent==GetWindow(hFound, GW_OWNER)){
						hOldMiniPanelProc = (WNDPROC)SetWindowLongPtr(hFound, GWLP_WNDPROC, (LONG_PTR)MiniPanelProc);
						break;
					}
					hFound = FindWindowEx(NULL, hFound, "#32770", NULL);
				}
